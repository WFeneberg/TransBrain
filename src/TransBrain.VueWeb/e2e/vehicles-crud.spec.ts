...
test('adminUser_createEditAndDeleteVehicle_throughTheUi', async ({ page }) => {
    // See drivers.spec.ts for why login must be established from '/' - the OIDC redirect_uri
    // always lands on '/callback', which replaces the URL with '/' (see AuthCallback.vue), and
    // '/' already mounts the vehicle list directly.
    await page.goto('/');
