...
/// and this is the copy to delete. The rule list is duplicated from
/// CreateOrderCommandValidator rather than shared through a base class, so each command's
/// contract stays readable in one file; if a third order command needs the same list, revisit
/// that decision.
/// </remarks>
