...
public sealed class CreateOrderCommandValidator : AbstractValidator<CreateOrderCommand>
{
    ...
    {
        // AddressPayload is a non-nullable record parameter, but nullable reference types are not
        // enforced at runtime: a JSON body omitting "consignor" deserialises it to null. Without
        // these NotNull rules and the When guards, FluentValidation throws NullReferenceException
        // while evaluating the nested member access and the endpoint returns 500 instead of 400.
        // The When wrapper does not change PropertyName, so the dotted keys Task 7 and both
        // frontends map against are still "Consignor.Name" and friends.
